local M = {}
local config = require('mortepau.lsp.actions.config')
local util = require('mortepau.lsp.actions.util')
...
function M.rename()
  ...
  util.buf_autocmd(prompt_buf, 'LspRename', {
    { 'WinLeave', string.format([[lua require('mortepau.lsp.actions.util').close_float(%d, %d)]], prompt_buf, prompt_win) },
    { 'CursorMovedI', string.format([[lua require('mortepau.lsp.actions.rename').update(%d, %d)]], prompt_buf, prompt_win) },
  })
  ...
  for _, key in ipairs(cancel_keys) do
    vim.nnoremap(key, { util.close_float, prompt_buf, prompt_win }, { silent = true, buffer = prompt_buf })
  end
  vim.cmd('startinsert!')
end
